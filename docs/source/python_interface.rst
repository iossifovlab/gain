...
The full ``adapter.py`` file contains the standard wrapper code needed to expose this function as a GAIn annotator and can be downloaded :download:`here <files/adapter.py>`. In brief, it defines an ``ExperimentalFollowupAnnotator`` class, declares the ``experimental_followup`` attribute using ``AttributeSpec``, and implements both single-variant and batch annotation methods. Each of those answers an ``AggregatedValues`` -- a mapping keyed by the *name* of each configured attribute, read off ``self._attributes`` at annotation time, holding the attribute's final value. GAIn hands that mapping on as-is: it neither renames the keys nor reduces the values, so an annotator that aggregates does so itself before answering. With this wrapper in place, the plugin can be included in a pipeline like any other GAIn annotator.
