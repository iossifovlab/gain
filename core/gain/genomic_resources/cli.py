...
def _summarize_stale_schema(count: int) -> None:
    ...
    if count:
        logger.warning(
            ...
            "schema; rebuild them with `resource-stats -f`", count)
...
def _run_stats_core(
    ...
    for res in resources:
        ...
        try:
            ...
            if not (force or needs_rebuild):
                # The hash gate holds, dry run or not: the one state in
                # which a statistics file behind its writer's schema goes
                # unnoticed, so it is looked for here and nowhere else.
                # Independent of the derived gate too -- rewriting the
                # derived file cures nothing about the others.
    ...
    if dry_run:
        ...
        _summarize_stale_schema(schema_stale)
        return CommandResult(
