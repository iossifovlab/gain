...
def test_default_attributes() -> None:
    ...
    attrs = pipeline.annotators[0].attributes
    assert attrs[0].spec is not None
    assert attrs[0].spec.value_type == "str"
...
def test_default_remapping() -> None:
    ...
    attrs = pipeline.annotators[0].attributes
    assert attrs[1].spec is not None
    assert attrs[1].spec.value_type == "str"
