...
def test_a_covered_contig_outside_the_universe_degrades_the_global() -> None:
    display = build_coverage_display(
    ...
    # The contig the universe DOES bound keeps its own fraction: the
    # degradation is of the global, not of every row.
    assert [(row.chrom, row.fraction) for row in display.rows] == [
        ("chr1", 0.09), ("chrX", None)]


def test_a_universe_of_only_implausible_lengths_yields_raw_counts() -> None:
    display = build_coverage_display(
        "scores/one", _a_statistic({"chr1": 9}), {"chr1": 5, "chr2": 0})

    assert display.global_fraction is None
    assert display.uncovered is None
    assert not display.has_fractions
    assert [row.covered for row in display.rows] == [9]


def test_an_empty_universe_yields_raw_counts() -> None:
