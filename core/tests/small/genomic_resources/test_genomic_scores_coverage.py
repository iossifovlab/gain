...
def test_position_score_multiple_values_for_position() -> None:
    ...
    assert list(score.fetch_region_segments("1", 10, 20, ["score"])) == [
        (10, 15, [0.1]),
        (12, 18, [0.2]),
    ]
...
def test_allele_score_fetch_region_spanning_record() -> None:
    ...
    assert list(score.fetch_region_segments("1", 10, 20, ["freq"])) \
        == [(10, 10, [0.02])]
...
def test_allele_score_fetch_region_overlapping_positions() -> None:
    ...
    result = list(score.fetch_region_segments("1", 10, 11, ["freq"]))
    assert len(result) == 2
...
def test_position_score_fetch_region_all() -> None:
    ...
    result = [
        ...
        for rec in score.fetch_region_segments(chrom, None, None)
    ]
    assert len(result) == 4
