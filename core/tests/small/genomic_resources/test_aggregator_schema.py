...
``AGGREGATOR_SCHEMA`` validates the ``aggregator`` /
``aggregator`` field of a score in a
...
def _accepts(configured: Any) -> bool:
    ...
    validator = Validator({"aggregator": AGGREGATOR_SCHEMA})
    return bool(validator.validate({"aggregator": configured}))
...
def test_a_position_score_may_configure_count(tmp_path: pathlib.Path) -> None:
    ...
    assert score_def.aggregator == "count"
