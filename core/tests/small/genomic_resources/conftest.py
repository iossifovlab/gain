...
def named_allele_score(
    ...
    refusal has to NAME the resource: those go through a repository,
    because ``build_resource`` hands back the id ``""``, which cannot show
    that the id reached the message.  Unopened, because the refusal is
    raised where the score is built, and a test that also opens puts the
    open inside its own ``raises``.
    """
    repo = a_grr().with_resource(resource_id, builder).build_repo(tmp_path)
    return AlleleScore(repo.get_resource(resource_id))
