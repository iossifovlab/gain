...
def test_allele_score_fetch_region() -> None:
    ...
    assert list(score.fetch_region_segments_scores("1", 10, 11, ["freq"])) == \
        [(10, 10, [0.04]),
         (10, 10, [0.03]),
         (10, 10, [0.02])]

    assert list(score.fetch_region_segments_scores("1", 10, 16, ["freq"])) == \
        [(10, 10, [0.04]),
         (10, 10, [0.03]),
         (10, 10, [0.02]),
         (16, 16, [0.05]),
...
def test_allele_score_fetch_region_spanning_record_at_pos_begin() -> None:
    ...
    assert list(score.fetch_region_segments_scores("1", 10, 12, ["freq"])) \
        == [(10, 10, [0.02])]
