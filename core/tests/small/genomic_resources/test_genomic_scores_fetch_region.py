...
from gain.genomic_resources.genomic_scores.records import clip_to_region
...
def test_position_score_fetch_region(
    ...
    score_lines = list(
        position_score.fetch_region_segments_scores(
...
def test_position_score_fetch_region_does_not_check_consistency(
    ...
    assert len(list(position_score.fetch_region_segments_scores(
        chrom, begin, end))) \
        == 2
...
def test_np_score_fetch_regions(
    ...
    score_lines = list(
        np_score.fetch_region_segments_scores(
            "chr1", begin, end, scores=scores))
    assert len(score_lines) == len(expected)
    assert score_lines == expected
...
def test_np_score2_fetch_regions(
    ...
    score_lines = list(
        np_score2.fetch_region_segments_scores(
            "chr1", begin, end, scores=scores))
    assert len(score_lines) == len(expected)
    assert score_lines == expected
...
def test_allele_score_fetch_regions(
    ...
    score_lines = list(
        allele_score.fetch_region_segments_scores(
            "chr1", begin, end, scores=scores))
    assert len(score_lines) == len(expected)
    assert score_lines == expected
...
def test_reading_an_allele_score_going_backwards_raises_nothing(
    ...
    assert list(score.fetch_region_segments_scores("chr1", 1, 30)) == [
        (20, 20, [0.2]),
        (10, 10, [0.1]),
    ]


def test_fetch_region_segments_scores_reports_a_straddling_records_true_extent(
    position_score: PositionScore,
) -> None:
    # The records at (11, 13) and (21, 23) both straddle an edge of the
    # queried window [12, 22]; the segment read reports their own spans,
    # not the window's.
    assert list(position_score.fetch_region_segments_scores(
...
def test_clip_to_region_over_the_segment_read_reshapes_to_the_window(
    position_score: PositionScore,
) -> None:
    # The recipe a caller holding the clipped spans migrates to (gain#844
    # removed fetch_region_segment_scores, which was exactly this
    # composition): the straddling records above come back reshaped to the
    # window.
    assert list(clip_to_region(
        position_score.fetch_region_segments_scores(
            "chr1", 12, 22, scores=["s1"]),
        12, 22)) == [
        (12, 13, [1.0]),
...
def test_an_allele_segment_read_answers_at_the_record_position(
    tmp_path: pathlib.Path,
) -> None:
    # An allele read collapses each record to the point it sits at and
    # holds no window opinion: a ten-base deletion overlapping the window's
    # start answers at its own position, OUTSIDE the window.  Clipping is
    # the base kinds' history, not this one's -- and composing the clip
    # over the point drops it, which is the caller's call, not the read's.
    resource = (
        an_allele_score()
        .with_score("s", "float")
        .with_data("""
    ...
    assert list(score.fetch_region_segments_scores(
        "chr1", 15, 25, scores=["s"])) == [(10, 10, [0.1])]
    assert not list(clip_to_region(
        score.fetch_region_segments_scores("chr1", 15, 25, scores=["s"]),
        15, 25))
