...
from web_annotation.pipeline_cache import BuildCancelled, _await_build
...
async def test_await_build_resolves_real_future_result() -> None:
    ...
    assert await _await_build(future) == "done"
...
async def test_await_build_works_with_fake_future_result() -> None:
    ...
    immediately; _await_build must still settle its per-request waiter.
    """
    fake = FakeFuture("seq-result")
    assert await _await_build(cast(Future, fake)) == "seq-result"
...
async def test_await_build_works_with_fake_future_exception() -> None:
    ...
    with pytest.raises(RuntimeError, match="fake boom"):
        await _await_build(cast(Future, fake))
...
async def test_await_build_does_not_park_event_loop() -> None:
    ...
    result: str = await _await_build(future)
    await cheap_task

    assert result == "late"
    # The cheap coroutine made progress while _await_build was suspended.
...
async def test_await_build_cancelling_waiter_does_not_cancel_shared() -> None:
    ...
    task: asyncio.Task[str] = asyncio.ensure_future(_await_build(future))
    await asyncio.sleep(0.01)
    task.cancel()
